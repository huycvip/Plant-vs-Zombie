...
public class PotatoMineCard extends PlantCard {
    ...
    public PotatoMineCard(int X, int Y, JLayeredPane layeredPane) {
        ...
        PlantCardImage = new ImageIcon(getClass().getResource("/Image/PlantCardImage/PotatoMineCard.gif"));
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (cardX > grassX && cardY > grassY && cardX < grassX + grassWidth && cardY < grassY + grassHeight) {
            ...
            if (!gamePlay.getGrass(cardX, cardY).isAssigned() && gamePlay.getSunLabel().getCurrentSun() >= 25) {
                gamePlay.getGrass(cardX, cardY).setAssignedPlant(new PotatoMine((GamePlay) layeredPane));
            }
        }
        setLocation(X, Y);
    }
